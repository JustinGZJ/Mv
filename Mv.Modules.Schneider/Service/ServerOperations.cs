    ...
    public class ServerOperations : IServerOperations
    {
        ...
        public int Upload(UploadDataCollection uploaddataCollection)
        {
            ...
            foreach (var localdata in uploaddataCollection.UploadDatas)
            {
                ...
                localdata.Tensions.First().Result = localdata.Tensions.First().Values.Any() && localdata.Tensions.First().Values
                    .All(x => x.Value < localdata.Tensions.First().UpperLimit && x.Value > localdata.Tensions.First().LowerLimit);
                localdata.Tensions.Last().Result = localdata.Tensions.Last().Values.Any() && localdata.Tensions.Last().Values
                .All(x => x.Value < localdata.Tensions.Last().UpperLimit && x.Value > localdata.Tensions.Last().LowerLimit);
                File.WriteAllTextAsync(Path.Combine(Folders.TENSIONS, $"{Station}_{localdata.Code ?? ("Empty" + DateTime.Now.ToString("yyyyMMddHHmmss"))}.json"), JsonConvert.SerializeObject(localdata,Formatting.Indented));
                File.WriteAllTextAsync(Path.Combine(Folders.TENSIONSBACKUP, $"{Station}_{localdata.Code ?? ("Empty" + DateTime.Now.ToString("yyyyMMddHHmmss"))}.json"), JsonConvert.SerializeObject(localdata,Formatting.Indented));
            }
